...
static void current_stack_region(address *bottom, size_t *size) {
  pthread_attr_t attr;
  int res = pthread_getattr_np(pthread_self(), &attr); // pthread_getattr_np（）函数初始化attr引用的thread attributes对象，使其包含描述正在运行的线程的实际属性值。
  if (res != 0) {
    if (res == ENOMEM) {
  ...
  size_t stack_bytes;
  res = pthread_attr_getstack(&attr, (void **) &stack_bottom, &stack_bytes); //获取线程的堆栈信息（栈地址和栈大小）
  if (res != 0) {
  ...
  // guard pages where present.  We need to trim these off.                    pthread_attr_getstack（）返回的内存块包括存在的保护页。我们得把这些剪掉。
  size_t page_bytes = os::Linux::page_size();
  assert(((intptr_t) stack_bottom & (page_bytes - 1)) == 0, "unaligned stack");

  size_t guard_bytes;
  res = pthread_attr_getguardsize(&attr, &guard_bytes);                      //获取线程的栈保护区大小
  if (res != 0) {
    fatal(err_msg("pthread_attr_getguardsize failed with errno = %d", res));
  }
  int guard_pages = align_size_up(guard_bytes, page_bytes) / page_bytes;     //align_size_up主要是字节对齐用的
  ...
  pthread_attr_destroy(&attr); //销毁一个线程属性对象           pthread_attr_init -> 初始化一个线程对象的属性
