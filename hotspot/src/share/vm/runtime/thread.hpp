...
protected:
  // OS data associated with the thread   与线程关联的操作系统数据
  OSThread* _osthread;  // Platform-specific thread information

  // Thread local resource area for temporary allocation within the VM  用于在虚拟机内临时分配的线程本地资源区域
  ResourceArea* _resource_area;

  DEBUG_ONLY(ResourceMark* _current_resource_mark;)

  // Thread local handle area for allocation of handles within the VM   用于在虚拟机内分配句柄的线程本地句柄区域
