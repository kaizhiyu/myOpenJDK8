...
// See orderAccess.hpp.  We assume throughout the VM that mutex lock and   请参阅orderAccess.hpp。
// try_lock do fence-lock-acquire, and that unlock does a release-unlock,  我们假设在整个VM中，mutex lock和try_lock执行fence lock acquire，unlock执行release unlock，*按这个顺序*。
// *in that order*.  If their implementations change such that these
// assumptions are violated, a whole lot of code will break.               如果它们的实现发生了变化，从而违反了这些假设，那么大量代码将被破坏。

// The default length of monitor name is chosen to be 64 to avoid false sharing.  监视器名称的默认长度选择为64，以避免错误共享。
static const int MONITOR_NAME_LEN = 64;

class Monitor : public CHeapObj<mtInternal> {

 public:
  // A special lock: Is a lock where you are guaranteed not to block while you are  一个特殊的锁：是一个保证在你持有它的时候不会被阻塞的锁，也就是说，没有vm操作可以发生，获取其他锁等等。
  // holding it, i.e., no vm operation can happen, taking other locks, etc.         注：等级“特殊”必须是最低的（最早的）（除了“事件”？）使死锁声明正常工作。
  ...
  // The rank native is only for use in Mutex's created by JVM_RawMonitorCreate,    rank native仅用于由JVM RawMonitorCreate创建的互斥体中，该互斥体不受死锁检测的影响。
  // which being external to the VM are not subject to deadlock detection.
  // The rank safepoint is used only for synchronization in reaching a              秩安全点仅用于到达安全点和离开安全点时的同步。
  // safepoint and leaving a safepoint.  It is only used for the Safepoint_lock     目前仅用于安全点锁。而在安全点上，任何线程都不持有级别安全点的互斥体。
  // currently.  While at a safepoint no mutexes of rank safepoint are held
  // by any thread.
  // The rank named "leaf" is probably historical (and should                       名为“leaf”的列组可能是历史性的（应该更改）——这个列组的互斥体根本不是真正的叶互斥体。
  ...
  // The WaitSet and EntryList linked lists are composed of ParkEvents.  WaitSet和EntryList链接列表由ParkEvents组成。
  // I use ParkEvent instead of threads as ParkEvents are immortal and   我使用ParkEvent而不是线程，因为ParkEvents是不朽的并且类型是稳定的，
 ...
 protected:                              // Monitor-Mutex metadata 监控元数据
  ...
  Thread * volatile _owner;              // The owner of the lock
                                         ...
                                         // to aid future synchronization mechanisms.  锁的所有者考虑在自己的$行上隔离所有者，以帮助将来的同步机制。
  ParkEvent * volatile _EntryList ;      // List of threads waiting for entry
  ParkEvent * volatile _OnDeck ;         // heir-presumptive   推定继承人
  volatile intptr_t _WaitLock [1] ;      // Protects _WaitSet
  ParkEvent * volatile  _WaitSet ;       // LL of ParkEvents
  volatile bool     _snuck;              // Used for sneaky locking (evil).
  int NotifyCount ;                      // diagnostic assist
  char _name[MONITOR_NAME_LEN];          // Name of mutex 互斥锁名字
