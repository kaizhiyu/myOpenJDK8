...
extern Monitor* Threads_lock;                    // a lock on the Threads table of active Java threads    活动Java线程的Threads表上的锁
                                                 // (also used by Safepoints too to block threads creation/destruction)  （也被安全点用于阻止线程创建/销毁）
extern Monitor* CGC_lock;                        // used for coordination between                         用于前后台GC线程之间的协调。
...
// A MutexLocker provides mutual exclusion with respect to a given mutex     MutexLocker为包含锁的作用域提供对给定互斥对象的互斥。
// for the scope which contains the locker.  The lock is an OS lock, not     锁是一个操作系统锁，而不是一个对象锁，并且两者不互操作。
// an object lock, and the two do not interoperate.  Do not use Mutex-based  不要使用基于互斥锁的锁来锁定Java对象，因为如果使用Java锁定机制锁定某个对象，则不会考虑这些对象。
...
//                NOTE WELL!!
//
// See orderAccess.hpp.  We assume throughout the VM that MutexLocker's      查看oederAccess.hpp，我们假设在整个VM期间，MutexLocker和友元构造函数执行fence,锁以及以一定顺序的需求
// and friends constructors do a fence, a lock and an acquire *in that
// order*.  And that their destructors do a release and unlock, in *that*    然后在它们的析构函数中释放、解锁以一定顺序，就是上面提到的顺序.
// order.  If their implementations change such that these assumptions       如果它们的实现发生了变化，从而违反了这些假设，那么大量代码将被破坏。
// are violated, a whole lot of code will break.

// Print all mutexes/monitors that are currently owned by a thread; called
// by fatal error handler.  打印线程当前拥有的所有互斥/监视器；由致命错误处理程序调用。
...
// A MutexLockerEx behaves like a MutexLocker when its constructor is       当使用互斥锁调用互斥锁的构造函数时，互斥锁的行为类似于互斥锁。
// called with a Mutex.  Unlike a MutexLocker, its constructor can also be  与MutexLocker不同，它的构造函数也可以用NULL调用，在这种情况下，MutexLockerEx是一个no-op。
// called with NULL, in which case the MutexLockerEx is a no-op.  There     还有一个相应的mutexunlockerx。
// is also a corresponding MutexUnlockerEx.  We want to keep the            我们希望保持基本的互斥锁的速度尽可能快。
// basic MutexLocker as fast as possible.  MutexLockerEx can also lock      MutexLockerEx也可以在没有安全点检查的情况下锁定。
