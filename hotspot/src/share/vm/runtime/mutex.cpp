...
int Monitor::TryFast () {
  ...
  // Fast-path attempt for the common uncontended case. 常见的无争议案件
...
// ON THE VMTHREAD SNEAKING PAST HELD LOCKS: 在VMTHREAD偷偷经过持有的锁时：
// In particular, there are certain types of global lock that may be held  特别是，当Java线程在安全点被阻止时，但在它写入“所有者”字段之前，它可能持有某些类型的全局锁。
// by a Java thread while it is blocked at a safepoint but before it has
// written the _owner field. These locks may be sneakily acquired by the   在安全点期间，VM线程可能会偷偷获取这些锁，以避免死锁。
// VM thread during a safepoint to avoid deadlocks. Alternatively, one should
// identify all such locks, and ensure that Java threads never block at    或者，应该识别所有这样的锁，并确保Java线程在保存它们的时候不会在安全点处阻塞（不安全点检查标志）。
// safepoints while holding them (_no_safepoint_check_flag). While it      虽然看起来这可能会增加到达安全点的时间（或者至少增加平均值，如果不是方差的话），但后一种方法可能会使JVM设计更干净、更易于维护。
...
//
// Sneaking is vile and reprehensible and should be excised at the 1st    偷偷摸摸是卑鄙和应受谴责的，应该在第一次机会就被除掉。
...
void Monitor::lock (Thread * Self) {
  ...
  // or GC threads. 清除未处理的oops，这样我们马上就会崩溃。仅对非vm或GC线程清除。
