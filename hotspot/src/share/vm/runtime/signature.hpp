...
class SignatureIterator: public ResourceObj {
 protected:
  Symbol*      _signature;             // the signature to iterate over  要迭代的签名
  int          _index;                 // the current character index (only valid during iteration) 当前字符索引（仅在迭代期间有效）
  int          _parameter_index;       // the current parameter index (0 outside iteration phase)   当前参数索引（0在迭代阶段之外）
  BasicType    _return_type;

  void expect(char c);
  void skip_optional_size();
  int  parse_type();                   // returns the parameter size in words (0 for void)  以字为单位返回参数大小（0表示void）
