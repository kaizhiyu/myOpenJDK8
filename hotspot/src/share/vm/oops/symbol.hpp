...
class SymbolBase : public MetaspaceObj {
 public:
  ATOMIC_SHORT_PAIR(
    volatile short _refcount,  // needs atomic operation 需要原子操作
    unsigned short _length     // number of UTF8 characters in the symbol (does not need atomic op) 符号中的UTF8字符数（不需要原子运算）
  );
  int            _identity_hash;
};
