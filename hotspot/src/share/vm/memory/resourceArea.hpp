...
// when the destructor is called.  Typically used as a local variable. 当调用析构函数时，资源标记释放在构造之后分配的所有资源。通常用作局部变量。
class ResourceMark: public StackObj {
protected:
  ResourceArea *_area;          // Resource area to stack allocate  要堆栈分配的资源区域
  Chunk *_chunk;                // saved arena chunk  保存的竞技场区块
  char *_hwm, *_max;
