...
// Base class for objects stored in Metaspace. 存储在元空间中的对象的基类。
// Calling delete will result in fatal error.  调用delete将导致致命错误。
//
// Do not inherit from something with a vptr because this class does        不要从带有vptr的对象继承，因为这个类没有引入vptr。
// not introduce one.  This class is used to allocate both shared read-only 该类用于分配共享只读和共享读写类。
...
class Chunk: CHeapObj<mtChunk> {
 ...
 protected:
  Chunk*       _next;     // Next Chunk in list  Chunk链中下一个Chunk实例
  const size_t _len;      // Size of this Chunk  当前Chunk的内存大小
 public:
...
//------------------------------Arena------------------------------------------
// Fast allocation of memory  是一个支持快速分配内存的基类，最终内存分配是通过Chunk完成的
...
protected:
  ...
  MEMFLAGS    _flags;           // Memory tracking flags  MEMFLAGS，表示内存的类型

  Chunk *_first;                // First chunk    Chunk链中第一个Chunk
  Chunk *_chunk;                // current chunk  Chunk链中当前使用的Chunk
  char *_hwm, *_max;            // High water mark and max in current chunk  当前Chunk的可分配内存的边界，即_hwm和_max之间的内存是未分配的
  // Get a new Chunk of at least size x
  void* grow(size_t x, AllocFailType alloc_failmode = AllocFailStrategy::EXIT_OOM);
  size_t _size_in_bytes;        // Size of arena (used for native memory tracking)  当前Area已经使用掉的内存大小，用于跟踪内存使用情况
