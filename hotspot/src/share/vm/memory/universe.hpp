...
// Universe is a name space holding known system classes and objects in the VM. Universe是一个名称空间，在VM中保存已知的系统类和对象。
//
// Loaded classes are accessible through the SystemDictionary. 加载的类可以通过SystemDictionary访问。
//
...
// and bails out to Scavenge::invoke_and_allocate. 对象堆通过Universe进行分配和访问，并提供各种分配支持。由解释器和编译后的代码进行内联分配，并释放以清除：：invoke_和_allocate。
...
class Universe: AllStatic {
 ...
 public:
  ...
  // as static fields of "Universe". 对作为“Universe”的静态字段维护的所有直接堆指针的地址应用“f”。
