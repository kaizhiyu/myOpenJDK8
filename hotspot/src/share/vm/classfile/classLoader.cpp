...
bool ClassLoader::add_package(const char *pkgname, int classpath_index, TRAPS) {
  ...
  // load_instance_class and thereby add_package
  {
    ...
    // First check for previously loaded entry  根据包名找到所对应的PackageInfo
    PackageInfo* pp = lookup_package(pkgname);
    if (pp != NULL) {
      // Existing entry found, check source of package  如果找到的话,则设置classpath_index
      pp->set_index(classpath_index);
      return true;
    }
    // strrchr 函数功能：查找一个字符c在另一个字符串str中末次出现的位置（也就是从str的右侧开始查找字符c首次出现的位置），并返回这个位置的地址。如果未能找到指定字符，那么函数将返回NULL。使用这个地址返回从最后一个字符c到str末尾的字符
    const char *cp = strrchr(pkgname, '/');  // 如果没有找的的话  获得包名,如此时为java/lang/Class.class ,则cp 为java/langs
    if (cp != NULL) {
      // Package prefix found  计算包名的长度
      int n = cp - pkgname + 1;

      char* new_pkgname = NEW_C_HEAP_ARRAY(char, n + 1, mtClass);  // 在c堆中分配数组,如果失败,则返回false
      if (new_pkgname == NULL) {
        return false;
      }

      memcpy(new_pkgname, pkgname, n);  // 添加到_package_hash_table 中
      new_pkgname[n] = '\0';
...
instanceKlassHandle ClassLoader::load_classfile(Symbol* h_name, TRAPS) {
  ...
  st.print_raw(h_name->as_utf8()); // 获得对应的文件名
  st.print_raw(".class");
  const char* file_name = st.as_string();
  ClassLoaderExt::Context context(class_name, file_name, THREAD);

  // Lookup stream for parsing .class file 加载对应的文件流
  ...
  if (stream != NULL) {
    // class file found, parse it  解析类文件
