...
// The ProtectionDomainCacheTable contains all protection domain oops. The system  ProtectionDomainCacheTable包含所有保护域oop。
// dictionary entries reference its entries instead of having references to oops   系统字典条目引用其条目，而不是直接引用oops。
// directly.
// This is used to speed up system dictionary iteration: the oops in the           这用于加快系统字典的迭代：保护域中的oop是唯一引用Java堆的oop。
// protection domain are the only ones referring the Java heap. So when there is   因此，当需要更新它们时，我们只需要遍历这个集合，而不是遍历系统字典的每个条目。
// need to update these, instead of going over every entry of the system dictionary,
// we only need to iterate over this set.
// The amount of different protection domains used is typically magnitudes smaller 使用的不同保护域的数量通常小于系统字典条目（加载的类）的数量。
