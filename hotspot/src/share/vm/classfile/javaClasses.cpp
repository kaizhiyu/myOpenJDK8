...
// based on non-null field   对于类装入器类，基于非空字段定义的可并行
...
oop java_lang_ClassLoader::non_reflection_class_loader(oop loader) {
  if (loader != NULL) {
    // See whether this is one of the class loaders associated with               查看这是否是与生成的用于反射的字节码关联的类装入器之一，
    // the generated bytecodes for reflection, and if so, "magically"             如果是的话，
    // delegate to its parent to prevent class loading from occurring             神奇地将类委托给其父类，以防止类加载发生在使用反射的应用程序不希望发生此类加载的地方。
