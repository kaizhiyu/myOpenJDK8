...
// ----------------------------------------------------------------------------
// Parallel class loading check  并行类加载检查
...
Klass* SystemDictionary::resolve_instance_class_or_null(Symbol* name,
  ...
  // defining the class in parallel by accident.       非引导类加载器将调用类加载器并通过jvm/jni-define class进行定义，jvm/jni-DefineClass将获取类加载器对象锁，以防止意外地并行定义类的多个线程。
  // This lock must be acquired here so the waiter will find
  // any successful result in the SystemDictionary and not attempt
  // the define                                                         必须在此处获取此锁，这样等待的线程才能在SystemDictionary中找到任何成功的结果，并且不会尝试定义.
  // ParallelCapable Classloaders and the bootstrap classloader,
  // or all classloaders with UnsyncloadClass do not acquire lock here  可并行的类加载器和引导类加载器，或者所有具有非同步加载类的类加载器都不会在此处获取锁。
  ...
  // Class is not in SystemDictionary so we have to do loading.  在SystemDictionary中类不存在,因此我们需要进行加载
  // Make sure we are synchronized on the class loader before we proceed  在我们处理之前,我们必须进行同步
  ...
  // Check again (after locking) if class already exist in SystemDictionary  在获得锁后再次进行检查该类是否存在
  ...
  {
    MutexLocker mu(SystemDictionary_lock, THREAD);
    Klass* check = find_class(d_index, d_hash, name, loader_data);  // 查找该类是否已经在Dictionary中
    if (check != NULL) {
      // Klass is already loaded, so just return it  如果已经存在的话,则直接返回
      class_has_been_loaded = true;
      k = instanceKlassHandle(THREAD, check);
    } else {
      placeholder = placeholders()->get_entry(p_index, p_hash, name, loader_data); // 如果在Dictionary中不存在,但是正在加载父类的过程中
      if (placeholder && placeholder->super_load_in_progress()) {
  ...
  // If the class is in the placeholder table, class loading is in progress  如果在Dictionary中不存在,但是正在加载父类的过程中
  if (super_load_in_progress && havesupername==true) {
    k = SystemDictionary::handle_parallel_super_load(name, superclassname,       // 处理并行加载
        class_loader, protection_domain, lockObject, THREAD);
    if (HAS_PENDING_EXCEPTION) {  // 如果有错误发生, 则返回null,否则则代表类已加载成功
      return NULL;
    }
    if (!k.is_null()) {           // 如果k非空, 则设置class_has_been_loaded为true,意味着类已加载成功
      class_has_been_loaded = true;
    }
  }
  ...
  if (!class_has_been_loaded) {
    ...
    // add placeholder entry to record loading instance class  添加placeholder来记录加载中的instance class（需要标记的是，一旦成功加载之后会将placeholder中的记录删除，由于随时可能被删除，因此需要加锁）
    // Five cases:
    // All cases need to prevent modifying bootclasssearchpath
    // in parallel with a classload of same classname          所有情况都需要防止与同名的类加载同时修改bootclasssearchpath。
    // Redefineclasses uses existence of the placeholder for the duration
    // of the class load to prevent concurrent redefinition of not completely
    // defined classes.                                        redefineclasses在类加载期间使用占位符的存在来防止未完全定义的类的并发重定义
    // case 1. traditional classloaders that rely on the classloader object lock  依赖类加载器对象锁的传统类加载器-不需要加载实例
    ...
    //    completes the load and other requestors wait for completion.  传统的类加载器将类加载器对象锁作为死锁解决方案来断开。检测这种情况需要在保持类加载器对象锁的同时执行此检查，并且在调用类加载器的LoadClass时仍然保持该锁。对于这些类加载器，我们确保第一个请求者完成加载，其他请求者等待完成。
    // case 3. UnsyncloadClass - don't use objectLocker
    //    With this flag, we allow parallel classloading of a
    //    class/classloader pair                                        不要将objectlocker与此标志一起使用，我们允许类/类加载器对的并行类加载。
    // case4. Bootstrap classloader - don't own objectLocker
    //    This classloader supports parallelism at the classloader level,
    //    but only allows a single load of a class/classloader pair.
    //    No performance benefit and no deadlock issues.                引导类加载器-不拥有ObjectLocker这个类加载器支持类加载器级别的并行性，但只允许类/类加载器对的单个加载。没有性能优势，也没有死锁问题。
...

    {
      MutexLocker mu(SystemDictionary_lock, THREAD);
      if (class_loader.is_null() || !is_parallelCapable(class_loader)) {
        PlaceholderEntry* oldprobe = placeholders()->get_entry(p_index, p_hash, name, loader_data);  // 从placeholders 中 查找对应的PlaceholderEntry
        if (oldprobe) {
          ...
          if (oldprobe->check_seen_thread(THREAD, PlaceholderTable::LOAD_INSTANCE)) {
            ...
            while (!class_has_been_loaded && oldprobe && oldprobe->instance_load_in_progress()) {
              ...
              // wait on first requestor                                            防止无效的类加载，等待bootstrap classloader 加载完毕
              if (class_loader.is_null()) {
                SystemDictionary_lock->wait();
              } else {
              ...
              // check if other thread failed to load and cleaned up  检查是否有其他线程加载失败并且进行清除
      ...
      if (!throw_circularity_error && !class_has_been_loaded) {
        PlaceholderEntry* newprobe = placeholders()->find_and_add(p_index, p_hash, name, loader_data, PlaceholderTable::LOAD_INSTANCE, NULL, THREAD);  // 添加到placeholders中
        load_instance_added = true;
        // For class loaders that do not acquire the classloader object lock,   对于不获取类加载器对象锁的类加载器，
        // if they did not catch another thread holding LOAD_INSTANCE,          如果他们没有捕捉到另一个持有LOAD_INSTANCE的线程，就需要一个类似于acquire ObjectLocker/find_class的检查
        // need a check analogous to the acquire ObjectLocker/find_class
        // i.e. now that we hold the LOAD_INSTANCE token on loading this class/CL  也就是说，既然我们在装载这个类/cl时持有LOAD_INSTANCE令牌，
        // one final check if the load has already completed
        // class loaders holding the ObjectLock shouldn't find the class here   那么最后检查一下，如果装载已经完成，那么持有objectlock的类装入器不应该在这里找到该类。
        Klass* check = find_class(d_index, d_hash, name, loader_data);
        if (check != NULL) {
        // Klass is already loaded, so return it after checking/adding protection domain  类加载结束,直接返回
          k = instanceKlassHandle(THREAD, check);
          class_has_been_loaded = true;
        }
      }
    ...
    // must throw error outside of owning lock  必须在释放锁的地方抛出异常
    if (throw_circularity_error) {
    ...
    if (!class_has_been_loaded) {
      ...
      // For UnsyncloadClass only   对于非异步加载类的情况
      // If they got a linkageError, check if a parallel class load succeeded.  如果获得的异常时linkageError,则检查是否存在并行加载成功的情况
      // If it did, then for bytecode resolution the specification requires     如果是这样，那么对于字节码解析，规范要求我们返回与其他线程相同的结果
    ...
    if (load_instance_added == true) {
      ...
      // and initiating loaders   不管是成功还是失败,都清理placeholder 中的entry.与此同时,还清除调用load_instance_class 加载父类失败时的LOAD_SUPERentries
      MutexLocker mu(SystemDictionary_lock, THREAD);
...
Handle SystemDictionary::compute_loader_lock_object(Handle class_loader, TRAPS) {
  // If class_loader is NULL we synchronize on _system_loader_lock_obj  如果类装入器为空，则在_system_loader_lock_obj上同步
  if (class_loader.is_null()) {
...
void SystemDictionary::initialize(TRAPS) {
  ...
  // Allocate private object used as system class loader lock  分配用作系统类加载程序锁的私有对象
  _system_loader_lock_obj = oopFactory::new_intArray(0, CHECK);
  // Initialize basic classes 初始化基本类
  initialize_preloaded_classes(CHECK);
}

// Compact table of directions on the initialization of klasses: klasses初始化的紧凑方向表：
...
void SystemDictionary::initialize_preloaded_classes(TRAPS) {
  ...
  // Preload commonly used klasses  预加载通用的类
