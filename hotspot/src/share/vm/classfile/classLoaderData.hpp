...
// produce.  类装入器表示链接集。从概念上讲，链接集标识动态链接器可以生成的已解析链接的完整传递闭包。
//
...
// the types it defines.  ClassLoaderData还封装了称为metaspace的分配空间，动态链接器使用该空间来分配其定义的所有类型的运行时表示。
//
...
// for root tracing and other GC operations.  ClassLoaderData存储在类和系统字典的运行时表示中，是垃圾收集的根，并为根跟踪和其他GC操作提供迭代器。
...
// GC root for walking class loader data created  已创建用于行走类加载器数据的GC根目录
...
class ClassLoaderData : public CHeapObj<mtClass> {
 ...
 private:
  ...
  oop _class_loader;          // oop used to uniquely identify a class loader
                              // class loader or a canonical class path  用于唯一标识类装入器类装入器或规范类路径的oop
  Dependencies _dependencies; // holds dependencies from this class loader
                              // data to others.

  Metaspace * _metaspace;  // Meta-space where meta-data defined by the
                           // classes in the class loader are allocated.  该ClassLoader为加载的Klass分配内存的Metaspace
  Mutex* _metaspace_lock;  // Locks the metaspace for allocations and setup. 分配内存的锁
  bool _unloading;         // true if this class loader goes away         为true，表示被卸载了
  bool _keep_alive;        // if this CLD is kept alive without a keep_alive_object().  为true，表示ClassLoaderData是活跃的但是没有关联的活跃对象，比如匿名类的类加载器和启动类加载器实例，为true则不能被GC回收掉
  bool _is_anonymous;      // if this CLD is for an anonymous class       为true，表示ClassLoaderData主要加载匿名类
  volatile int _claimed;   // true if claimed, for example during GC traces.  用来标记该ClassLoaderData已经被遍历过了
                           // To avoid applying oop closure more than once.
                           // Has to be an int because we cas it.
  Klass* _klasses;         // The classes defined by the class loader.    该ClassLoaderData加载的所有Klass

  JNIHandleBlock* _handles; // Handles to constant pool arrays 保存所有常量池的引用
