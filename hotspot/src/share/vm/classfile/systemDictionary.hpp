...
// The system dictionary stores all loaded classes and maps:                系统字典存储所有加载的类和映射：
//
//   [class name,class loader] -> class   i.e.  [Symbol*,oop] -> Klass*
//
// Classes are loaded lazily. The default VM class loader is                类被延迟加载。默认的VM类加载器表示为空。
// represented as NULL.
...
// but only by different loaders. 底层数据结构是一个具有固定数量存储桶的开放哈希表。加载期间，加载程序对象被锁定（对于VM加载程序，使用私有锁对象）。因此，类加载可以同时进行，但只能由不同的加载程序进行。
//
// During loading a placeholder (name, loader) is temporarily placed in  在加载期间，占位符（name，loader）被临时放置在一个side数据结构中，用于检测类循环错误并在GC期间执行验证。
...
// of class loading, as we call out to Java, have to take locks, etc.    GC可能发生在类加载的过程中，就像我们调用Java一样，必须获取锁等。
//
// When class loading is finished, a new entry is added to the system    当类加载完成时，系统字典中会添加一个新条目，并移除占位符。
// dictionary and the place holder is removed. Note that the protection  请注意，在创建“真实”系统字典条目时，尚未填写系统字典的“保护域”字段。
// domain field of the system dictionary has not yet been filled in when
// the "real" system dictionary entry is created.
//
// Clients of this class who are interested in finding if a class has    这个类的客户机，他们有兴趣发现一个类是否已完全加载, 加载过程中的类不能读取未锁定的SystemDictionary。
...
//         readers (we must ensure write ordering)    在可以被并发读之前所有项需要完全形成
//
...
// of placeholders must hold the SystemDictionary_lock. 请注意，占位符随时都会被删除，因为它们在类完全加载时会被删除。因此，占位符的读者和作者必须持有SystemDictionary锁。
//
...
// to provide a different definition.  某些类是预加载的，例如java.lang.Object和java.lang.String。它们都是“众所周知的”，因为不允许类加载器提供不同的定义。
//
// These klasses must all have names defined in vmSymbols.  这些klass必须都有vmSymbols中定义的名称。

#define WK_KLASS_ENUM_NAME(kname)    kname##_knum

// Each well-known class has a short klass name (like object_klass),  每个已知的类都有一个简短的klass名称（比如object_klass），
// a vmSymbol name (like java_lang_Object), and a flag word
// that makes some minor distinctions, like whether the klass
// is preloaded, optional, release-specific, etc.  vmSymbol名称（比如java_lang_对象）和一个标志词，可以做一些细微的区分，比如klass是否是预加载的、可选的、特定于发行版的等等。
// The order of these definitions is significant; it is the order in which
// preloading is actually performed by initialize_preloaded_classes.  这些定义的顺序很重要；它是通过初始化预加载的类实际执行预加载的顺序。
...
public:
  ...
  // Local definition for direct access to the private array: 直接访问私有数组的本地定义
...
protected:
  ...
  // Setup link to hierarchy  设置到层次结构的链接
  ...
  // Variables holding commonly used klasses (preloaded)  通用类（预加载）
