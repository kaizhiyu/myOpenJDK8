...
//////////////////////////////////////////////////////////////////////////////
// create new thread 创建新线程

static address highest_vm_reserved_address();

// check if it's safe to start a new thread 检查启动一个新线程是否安全
static bool _thread_safety_check(Thread* thread) {
  if (os::Linux::is_LinuxThreads() && !os::Linux::is_floating_stack()) {
    // Fixed stack LinuxThreads (SuSE Linux/x86, and some versions of Redhat) 固定堆栈LinuxThreads（SuSE Linux/x86和一些Redhat版本）堆位于内存空间的低端。
    //   Heap is mmap'ed at lower end of memory space. Thread stacks are      线程堆栈是从高地址空间分配的（映射是固定的）。
    //   allocated (MAP_FIXED) from high address space. Every thread stack    每个线程堆栈都占用一个固定大小的插槽（通常为2字节，但如果用户重建LinuxThreads，则可以将其更改为其他值）。
    ...
    // Floating stack LinuxThreads or NPTL:  浮动堆栈LinuxThreads或NPTL：
    //   Unlike fixed stack LinuxThreads, thread stacks are not MAP_FIXED. When  与固定堆栈LinuxThreads不同，线程堆栈不是映射固定的。
    //   there's not enough space left, pthread_create() will fail. If we come   当没有足够的空间时，pthread_create（）将失败。
    //   here, that means enough space has been reserved for stack.              如果我们来到这里，这意味着已经为stack预留了足够的空间。
    return true;
  }
}

// Thread start routine for all newly created threads 所有新创建线程的线程启动例程
static void *java_start(Thread *thread) {
  // Try to randomize the cache line index of hot stack frames. 尝试随机化热堆栈帧的缓存线索引。
  // This helps when threads of the same stack traces evict each other's 当相同堆栈跟踪的线程逐出彼此的缓存线时，这会有所帮助。
  // cache lines. The threads can be either from the same JVM instance, or 线程可以来自同一个JVM实例，也可以来自不同的JVM实例。
  // from different JVM instances. The benefit is especially true for 对于采用超线程技术的处理器来说，这一好处尤其明显。
  ...
  // non floating stack LinuxThreads needs extra check, see above 非浮动堆栈LinuxThreads需要额外检查，请参见上文
  ...
  // thread_id is kernel thread id (similar to Solaris LWP id) thread_id是内核线程id（类似于Solaris LWP id）
  ...
  // initialize signal mask for this thread  初始化此线程的信号掩码
  ...
  // call one more level start routine  再调用一个级别启动例程
  thread->run();
...
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  ...
  // Allocate the OSThread object 创建操作系统线程
  OSThread* osthread = new OSThread(NULL, NULL);
  if (osthread == NULL) {
    return false;
  }

  // set the correct thread state 设置正确的线程状态
  osthread->set_thread_type(thr_type);

  // Initial state is ALLOCATED but not INITIALIZED  把osthread状态设置为已分配
  osthread->set_state(ALLOCATED);

  thread->set_osthread(osthread); // 绑定至JavaThread

  // init thread attributes 初始化线程数形
  pthread_attr_t attr;
  ...
  if (os::Linux::supports_variable_stack_size()) {
    // calculate stack size if it's not specified by caller 如果调用方未指定堆栈大小，则计算堆栈大小
    if (stack_size == 0) {
    ...
    // let pthread_create() pick the default value. 让pthread_create（）选择默认值。
  }
...

  {
    ...
    if (ret != 0) {
      ...
      // Need to clean up stuff we've allocated so far 需要清理我们目前分配的东西
